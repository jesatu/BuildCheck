...
| C20 | **Prebook tier advancement.** Book all 4 events at the same time and spend all 4 purchases at prebook (so all T1–T3, not `@`), and you can advance one of those skills a second level straight away, e.g. Dismiss/Control +4 then +6. Pay the OSP for both levels; you need the OSP for all 5 skills. The advancement can't be Tier 5 or restricted. Not in the same season as retirement double steps. | 8.2, 8.3 |
| C21 | **One special creature at a time.** The character's race must match Awakened <X>. A character can be only one of: an essence creature (one kind), awakened (one race), a ritual race (Daemon, Ancestral, Elemental, Beast, Plant, Mineral) or a changed pattern (Magical, or Unliving other than as a Vampire). **Exception:** a ritual race (Daemon, Ancestral, Elemental, Beast, Plant, Mineral) can have a changed pattern; the pattern only clashes with essence and awakened. Sources: HB Essence Creature and Awakened <X> ("prevents the character becoming ... any other kind of Special Creature"), the same line on every awakened loresheet, and PR-2 (1 special creature lammie). | 3, 8.6 EX-5, 12.3, 12.4 |
| C22 | **Awakened loresheet and skill are a package.** Every awakened loresheet comes with its Awakened <X> skill (gained through the Rite of Creation); neither can be held without the other. | 12.3 |
| C23 | **Sleepless Chanting is redundant** with any Sleep immunity (Immune to Sleep, Immune to Mind Effects, Mind Healing, Source of Life, Source of Unlife): its only effect is immunity to Sleep while chanting. Unending Voice stays active (Spell Reduction (1) on Chants). | 11 |
| C24 | **High Magic <X>** is one of Spellcasting, Incantation, Healing or Corruption, and needs Level 2 of that same CS to use (e.g. High Magic (Healing) needs Level 2 Healing). | 11 |
| C25 | **Not affected by.** Decay, Disease, Fatal and Paralysis only affect Living patterns; Smite, Bind Unliving and Enthral Unliving only affect Unliving patterns (HB Damage Effects, spells). A character of another pattern can't be affected by them at all, even with Elemental Weakness or Artefact. **Owner ruling (not in the rules):** the NPC/DPC loresheet means the holder can't be Beguiled. From the loresheets: **Unliving** – Ritual of Peace, non-magical alchemical poisons/potions; **Magical Pattern** – Ritual of Peace, alchemical and magical poisons/potions, Embody Unliving; **Alien Life Pattern** – Ritual of Peace, alchemical poisons/potions; **Paladin** (Blind Faith) – Heal Wound and effects based on it, Mage Armour cast on them; **Warlock** – Paladin's Armour cast on them; **Awakened Drow** – Wasting; **Awakened Human** – the Red Mist (Militia Guild) curse. | 5, 12 |
...
| C7 | **Jack of All Trades** is its own route. It needs the JoAT skill and **Oathsworn <guild>**, either bought or granted by the unpublished **NPC/DPC loresheet** (held for a faction or guild; it grants Oathsworn <X> and <X> Command). It teaches one skill of any tier (restricted included) from an Ω list of that guild, including the guild's group list; not High Magic <X>. Learn prerequisites, 4 purchases per year and one step per tree still apply. One use per season. **Only an Awakened Human can use it:** JoAT comes only from the Awakened Human loresheet, and each use removes it from the card, so every use buys it again there (20 OSP, one of that season's purchases). | 8.3 |
