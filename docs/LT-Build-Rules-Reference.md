...
- **OS-1** A character can buy at most **4 OS per event season**. This includes prebook and event purchases.
  ...
  - **Counts:** Awakened <X>.
- **OS-2** A character can move **only one step along any one OS tree per year**. The retirement exception is in [8.8](#88-retirement). Prebook is the other way to take two steps in one season (C20).
...
| C20 | **Prebook double step.** A skill self-taught at prebook (T1–T3, not `@`, bought normally) lets the next step on its tree be bought at a main event in the same season. Only two steps per tree per season this way; both count toward the 4 per year. | 8.2, 8.3 |
