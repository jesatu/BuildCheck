          ...
          <li key={i} className="row">
            ...
            {onSource && r.source !== 'granted' && (
              <select aria-label={`How ${s.name} was gained`} value={r.source === 'loresheet' ? `ls:${r.loresheet}` : r.source}
                ...
                {build?.loresheets.some((l) => l.id === 'awakened-human') && <option value="joat">Jack of All Trades</option>}
              ...
              <ul className="skills">
                {buys.map((p) => (
                  <li key={`${p.id}|${p.param}`} className="skill active">
                    ...
                    {(p.doubleStep || p.prebook || p.advanced || !p.countsTowardYearly || p.notes.length > 0) && (
                      <span className="detail">
                        {[p.doubleStep && 'Retirement double step', p.prebook && 'Prebook', p.advanced && 'Prebook tier advancement (5th purchase)', !p.countsTowardYearly && !p.doubleStep && !p.advanced && 'Doesn\'t use a yearly purchase', ...p.notes].filter(Boolean).join(' · ')}
                      </span>
                    )}
                  </li>
                ))}
